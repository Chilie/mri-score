...
class LangevinOptimizer(torch.nn.Module):
    def __init__(self, config, logger, project_dir='./', experiment=None):
        ...
        self.langevin_config.model.num_classes = self.config['total_iter'] #500
    ...
    def _sample(self, y):
        ...
        pbar = tqdm(range(self.langevin_config.model.num_classes-1), disable=(self.config['device'] != 0))
        ...
        diff = torch.ones_like(ref)
        meas_grad = torch.view_as_real(torch.sum(self._ifft(diff) * torch.conj(maps), axis=1) ).permute(0,3,1,2)
                # re-normalize, since measuremenets are from a normalized estimate
        meas_grad = normalize(meas_grad, estimated_mvue)
        # convert to float incase it somehow became double
        meas_grad = meas_grad.type(torch.cuda.FloatTensor)
        meas = forward_operator(normalize(meas_grad, estimated_mvue))
        scale = torch.norm(meas)**2/torch.norm(diff)**2
        # scale = meas.sum()/diff.sum()
        scale = scale.sqrt()
        ...
        with torch.no_grad():
            for c in pbar:
                ...
                sigma = self.sigmas[c]
                labels = torch.ones(samples.shape[0], device=samples.device) * c * 2311.0/self.config['total_iter']
                labels = labels.long()
                step_size = step_lr * (sigma / self.sigmas[-1]) ** 2

                sigma_pre = self.sigmas[c+1]
                # p_grad = self.score(samples, labels)
                step = 2*sigma*(sigma - sigma_pre)
                step_noise = (sigma**2 - sigma_pre**2)
                noise = torch.randn_like(samples)

                p_grad = self.score(samples, labels)

                # samples = samples + sigma**2*p_grad.detach()
                ...
                meas_grad = meas_grad/(scale*(sigma**2)*0.5)
                
                samples = samples + step_noise * (p_grad - meas_grad) + np.sqrt(step_noise)*noise
                ...
                if self.config['save_images']:
                    if (c+2) % self.config['save_iter'] ==0 :
                        ...
                        for i, exp_name in enumerate(self.config['exp_names']):
                            if self.config['repeat'] == 1:
                                file_name = f'{exp_name}_R={self.config["R"]}.jpg'
                                ...
                                for j in range(self.config['repeat']):
                                    file_name = f'{exp_name}_R={self.config["R"]}_sample={j}.jpg'
...
def mp_run(rank, config, project_dir, working_dir, files):
    ...
    for index, sample in enumerate(tqdm(loader)):
        ...
        # img_gen = normalize(samples, estimated_mvue)
        to_display = torch.view_as_complex(outputs[batch_idx]['mvue'].permute(0, 2, 3, 1).unsqueeze(1).contiguous()).abs().flip(-2).detach().cpu().numpy()
        to_display_mvue = mvue.abs().flip(-2).detach().cpu().numpy()
        # to_display = outputs[batch_idx]['mvue'].abs().flip(-2).detach().cpu().numpy()
        # display the qunatitive result
        b = (to_display - np.min(to_display))/np.ptp(to_display)
        c = (to_display_mvue - np.min(to_display_mvue))/np.ptp(to_display_mvue)
        psnr = compare_psnr(np.squeeze(b),np.squeeze(c))
        ssim = compare_ssim(np.squeeze(b),np.squeeze(c),data_range=1.0)
        
        print('image metric: PSNR {:.4f} and SSIM {:.4f}'.format(psnr,ssim))
        logger.info('{} image metric: PSNR {:.4f} and SSIM {:.4f}'.format(exp_name,psnr,ssim))
